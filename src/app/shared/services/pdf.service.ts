...
import { IOrder, IOrderItem } from '../classes/order';
...
// const { jsPDF }: typeof import('jspdf') = await import('jspdf');
...
export class PDFService {
  ...
  constructor(private tts: ToastrTranslateService) {}

  async exportInvoicePDF(order: IOrder) {
    const { jsPDF } = await import('jspdf'); // dynamically load module
  ...
  getInvoiceDetails(items: IOrderItem[]) {
    let prepareData: import('jspdf-autotable').RowInput[] = [];
  ...
  async exportReceiptPDF(order: IOrder) {
    const { jsPDF } = await import('jspdf'); // dynamically load module
    const { autoTable } = await import('jspdf-autotable');
    //const autoTable: typeof import('jspdf-autotable') = await import('jspdf-autotable');
