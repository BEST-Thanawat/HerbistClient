...
export class OrderService {
  ...
  constructor(private http: HttpClient, private tts: ToastrTranslateService) { }
  ...
  async exportInvoicePDF(order: IOrder) {
    ...
    doc.text(
      this.tts.getTranslate('Invoice Date') + ': ' +
        formatDate(order.orderDate, 'dd/MM/yyyy HH:mm', 'en-US'),
      40,
      145
    );
    doc.text(
      this.tts.getTranslate('Order Number') + ': ' + order.orderNumber,
      40,
      160
    );
    doc.text(
      this.tts.getTranslate('Order Date') + ': ' + 
        formatDate(order.orderDate, 'dd/MM/yyyy HH:mm', 'en-US'),
      40,
      175
    );
    ...
    const customerPosition =
      595 / 2 - doc.getTextDimensions(this.tts.getTranslate('Customer')).w / 2 - 50;
    doc.setFontSize(16);
    ...
    doc.text(
      order.shipToAddress.billing_Province +
        ' ' +
        order.shipToAddress.billing_ZipCode,
      customerPosition,
      285
    );
    ...
    doc.text(
      order.shipToAddress.province + ' ' + order.shipToAddress.zipCode,
      shippingPosition,
      285
    );
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        index: { halign: 'center', font: 'Noto' },
        productName: { halign: 'left', font: 'Noto' },
        amount: { halign: 'center', font: 'Noto' },
        price: { halign: 'center', font: 'Noto' },
        discount: { halign: 'center', font: 'Noto' },
        total: { halign: 'center', font: 'Noto' },
  ...
  async exportReceiptPDF(order: IOrder) {
    ...
    doc.text(
      this.tts.getTranslate('Receipt Date') + ': ' +
        formatDate(order.orderDate, 'dd/MM/yyyy HH:mm', 'en-US'),
      40,
      145
    );
