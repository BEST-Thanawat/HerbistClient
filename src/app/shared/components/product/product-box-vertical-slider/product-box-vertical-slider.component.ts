      ...
      next: (response) => {
        ...
        if (environment.cloudinary === true) {
          ...
          this.products!.forEach((element) => {
            ...
            const filename = element.images[0].src.substring(element.images[0].src.lastIndexOf('/') + 1);
            element.images[0].responsiveSrcSet = this.navService.GetProductResponsiveSrcSet('Products/' + filename);
          });
        }
      },
      error: (e) => {
